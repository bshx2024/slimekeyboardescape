# Dedicated Analysis Script for anomalousgame.io Backlinks

$inputCsv = 'C:\Users\69444\Downloads\anomalousgame.io-backlinks.csv'
$validCsv = 'f:\旅行携带资料\Games\slimekeyboardescape\anomalousgame_valid_backlinks.csv'
$spamCsv  = 'f:\旅行携带资料\Games\slimekeyboardescape\anomalousgame_spam_backlinks.csv'
$outputXlsx = 'f:\旅行携带资料\Games\slimekeyboardescape\anomalousgame_backlinks.xlsx'
...
foreach ($row in $data) {
    ...
    # Rule 5: Blogspot / General Zero AS Link Farm
    elseif (($domain -like "*.blogspot.com*" -and $ascore -eq 0) -or ($ascore -eq 0 -and $extLinks -gt 30)) {
        $isSpam = $true
        $spamReason = "零权重批量垃圾链接农场 (如 $domain)"
    }
    ...
    if ($isSpam) {
        ...
        if ($domain -like "*blog.naver.com*" -or $domain -like "*wordpress.com*" -or $domain -like "*blogspot.com*" -or $domain -like "*medium.com*" -or $sourceUrl -like "*/blog/*" -or $sourceTitle -like "*blog*") {
            ...
            $priority = "P0 (核心优先)"
            $actionPlan = "独立博客/玩家测评！在评论区发帖带网址，或联系作者要求测评你的游戏。"
        }
        elseif ($domain -like "*reddit.com*" -or $domain -like "*github.com*" -or $domain -like "*github.io*" -or $domain -like "*fandom.com*" -or $domain -like "*quora.com*" -or $domain -like "*discord*" -or $domain -like "*steamcommunity.com*" -or $sourceTitle -like "*forum*" -or $sourceTitle -like "*wiki*") {
            ...
            $priority = "P0 (核心优先)"
            $actionPlan = "高权重 UGC 社区！创建品牌/游戏展示页，在 Wiki 或帖子回复中嵌入游戏链接。"
        }
        elseif ($domain -like "*itch.io*" -or $domain -like "*gamejolt.com*" -or $domain -like "*poki.com*" -or $domain -like "*crazygames.com*" -or $sourceUrl -like "*unblocked*" -or $sourceTitle -like "*unblocked*" -or $sourceTitle -like "*directory*" -or $domain -like "*.io*") {
            $category = "2. 导航和资源目录 (Directories & Catalogs)"
            $priority = "P0 (核心优先)"
            $actionPlan = "游戏目录/IO游戏导航站！直接免费注册提交你的 slimekeyboardescape 链接入驻。"
        }
        else {
            if ($ascore -ge 15) {
...
Write-Host "Valid links count for anomalousgame.io: $($validSorted.Count)"
Write-Host "Cleaned spam links count for anomalousgame.io: $($spamSorted.Count)"
...
try {
    ...
    Write-Host "SUCCESS: Exported dedicated Excel file at $outputXlsx"
}
catch {
